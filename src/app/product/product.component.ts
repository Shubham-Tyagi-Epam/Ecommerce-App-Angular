...
export class ProductComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    private productImageContainer: ElementRef,
  ...
  addToCart() {
    ...
    if (this.authGuardService.canActivate()) {
      if (
        this.productType == 'MensFashion' ||
        this.productType == 'WomenFashion' ||
        this.productType == 'KidsFashion'
      ) {
  ...
  calculateOffSetLeftAndTop(eventType: string): void {
    const offSetTopAndLeft = cumulativeOffSet(
      this.productImageContainer.nativeElement,
      eventType
    );
